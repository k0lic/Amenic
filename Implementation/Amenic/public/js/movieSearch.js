...
const getMoviesAndCinemas = async () => {
	...
	if (aM) {
		path =
			...
			`/HomeController/cinemasSearch/${countryListSearch.selectedIndex}/${
				cityListSearch[cityListSearch.selectedIndex].value
			}/${searchBar.value}`;
	} else {
		path =
			`//` +
			window.location.host +
			`/HomeController/titleSearch?actMenu=${menu}&title=${searchBar.value}`;
	}
