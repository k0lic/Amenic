...
function searchForMoviesLike() {
    ...
    getMovieSearchResults(searchTerm).then((data) => { populateWithMovies(data); })
}

function searchForComingSoonsLike() {
    let searchTerm = searchBar.value;
    getSoonSearchResults(searchTerm).then((data) => { populateWithComingSoons(data); })
}

function searchForRoomsLike() {
    let searchTerm = searchBar.value;
    getRoomSearchResults(searchTerm).then((data) => { populateWithRooms(data); })
}

function searchForEmployeesLike() {
    let searchTerm = searchBar.value;
    getEmployeeSearchResults(searchTerm).then((data) => { populateWithEmployees(data); })
}
...
function movieTemplate(movie) {
    ...
    if (minutes < 10)
        minutes = "0" + minutes;
    return "<a class=\"coolLink\" href=\"/Cinema/EditMovie/" + movie.projection.idPro + "\">" +
        "<div class=\"movieImgExtended centerY column\">" +
        "<img src=\"" + movie.poster + "\" class=\"movieImg\" />" +
        "<div class=\"movieImgText row w80 mt-1 spaceBetween\">" +
        "<div>" + movie.projection.roomName + "</div>" +
        "<div>" + dayOfTheWeek + " " + hours + ":" + minutes + "</div>" +
        "</div>" +
        "</div>" +
        "</a>";
}
...
function movieCanceledTemplate(movie) {
    ...
    if (minutes < 10)
        minutes = "0" + minutes;
    return "<div class=\"movieImgExtended centerY column\">" +
...
function comingSoonTemplate(soon) {
    return "<a class=\"coolLink\" href=\"/Cinema/EditComingSoon/" + soon.soon.tmdbID + "\">" +
        "<div class=\"comingSoonExtended\">" +
        "<img src=\"" + soon.poster + "\" class=\"movieImg\" />" +
        "</div>" +
        "</a>";
}

function roomTemplate(room) {
    return "<a class=\"coolLink\" href=\"/Cinema/EditRoom/" + room.name + "\"</a>" +
        "<div class=\"movieImgExtended centerY column\">" +
...
function employeeTemplate(employee) {
    return "<div class=\"rowWrapper\">" +
        "<div class=\"userPicture\">" +
        "<img src=\"" + (employee.image == null ? "/assets/defaultUserImage.jpg" : "data:image/jpg;base64, " + employee.image) + "\" alt=\"Worker pic\" />" +
        "</div>" +
        "<div class=\"description\">" +
        "<div><h1>" + employee.worker.firstName + " " + employee.worker.lastName + "</h1></div>" +
        "<div><span>" + employee.worker.email + "</span></div>" +
        "</div>" +
        "<div class=\"binWrapper\">" +
        "<button type=\"button\"" +
        "onClick=\"document.getElementById('workerForDelete').value='" + employee.worker.email + "'; areYouSure('You are about to remove an employee.','/Cinema/ActionRemoveEmployee')\"" +
