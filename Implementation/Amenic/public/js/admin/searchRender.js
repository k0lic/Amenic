...
function createListElement(data, actMenu) {
    ...
    img.src = data.image == null ? "/assets/Admins/profile.jpeg" : "data:image/jpg;base64, " + data.image;
    ...
    name.innerHTML = !data.hasOwnProperty('name') ? data.firstName + " " + data.lastName : data.name;
    ...
    span.innerHTML = data.email
        + (!data.hasOwnProperty('address') || data.address == null ? "" : (" • " + data.address))
        + (!data.hasOwnProperty('cityName') || data.cityName == null ? "" : (" • " + data.cityName))
        + (!data.hasOwnProperty('countryName') || data.countryName == null ? "" : (" • " + data.countryName))
        + (!data.hasOwnProperty('phoneNumber') || data.phoneNumber == null ? "" : (" • " + data.phoneNumber));
...
const getUsers = async () => {
    ...
    let response = await fetch(
        `http://localhost:8080/AdminController/search`,
        {
            method: 'POST',
            body: fd,
            mode: "cors"
