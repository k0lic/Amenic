...
const galleryNewImagePreview = document.getElementById(
	"galleryNewImagePreview"
);
...
function movieRun(email, date, page) {
	getMovieRepertoire(email, date, page).then((data) => {
		saveFetchedItems(data);
		populateRepertoire();
	});
	updatePageNumberControls();
}
...
function comingSoonRun(email, page) {
	getComingSoonRepertoire(email, page).then((data) => {
		saveFetchedItems(data);
		populateRepertoire();
	});
	updatePageNumberControls();
...
function movieTemplate(projection) {
	let template =
		"" +
		'<div class="w30 column centerRow">' +
		projection.movieName +
		"</div>" +
		'<div class="w10 textCenter">' +
		...
		projection.type +
		"</div>" +
		'<div class="w15 textCenter">&euro;' +
		Number(projection.price).toFixed(2) +
		"</div>" +
		'<div class="w15 textCenter">' +
		projection.freeSeats +
		"</div>" +
		"</div>";
	...
	if (allowLinks) {
		template =
			"" +
			'<a class="anchorWrapper paintEveryOddItemsKid" href="/reservation/' +
			projection.idPro +
			'">' +
...
function soonTemplate(soon) {
	return (
		...
		'<div class="showingTableRow paintEveryOddItem row centerY mb-1">' +
		'<div class="w100 column centerRow">' +
		soon.movieName +
		"</div>" +
		"</div>"
	);
...
async function getMovieRepertoire(email, day, page) {
	...
	let response = await fetch(
		"//" +
			...
			"/Theatre/getMyRepertoire?cinemaEmail=" +
			email +
			"&day=" +
			day +
			"&page=" +
...
async function getComingSoonRepertoire(email, page) {
	let response = await fetch(
		"//" +
			window.location.host +
			"/Theatre/getMyComingSoons?cinemaEmail=" +
			email +
			"&page=" +
			page,
		{
...
async function countMovieRepertoire(email, day) {
	let response = await fetch(
		"//" +
			window.location.host +
			"/Theatre/countMyRepertoire?cinemaEmail=" +
			email +
			"&day=" +
			day,
		{
...
async function countComingSoonRepertoire(email) {
	let response = await fetch(
		"//" +
			window.location.host +
			"/Theatre/countMyComingSoons?cinemaEmail=" +
			email,
		{
			method: "GET",
			mode: "cors"
