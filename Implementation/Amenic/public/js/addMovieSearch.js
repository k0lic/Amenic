...
const movieSearchResultsContainer = document.getElementById(
...
function maxPageNumber() {
	if (totalResultsInHouse == null) {
		...
		return (
			1 +
			Math.floor(
				(totalResultsInHouse + totalResultsTMDB - duplicates - 1) / uiPageSize
			)
		);
	}
}
...
function setupListeners() {
	...
	movieNameInput.addEventListener("click", (e) => {
		e.stopPropagation();
	});
	movieSearchResultsContainer.addEventListener("click", (e) => {
		e.stopPropagation();
	});
...
function updateSearchTerm(newTerm) {
	searchTerm = newTerm;
	countHowManyMoviesLike(searchTerm).then((data) => {
		totalResultsInHouse = data;
	});
...
function pageChanged() {
	...
	if (
		firstMovieToShow < movieAfterLast() &&
		(movies.length <= firstMovieToShow || movies[firstMovieToShow] == null)
	) {
		getMoviesThenUpdateVarThenUpdateDom();
	} else {
...
function getTmdbMoviesThenUpdateVarThenUpdateDom() {
	...
	getMoviesLikeInTMDB(searchTerm, nextTMDBpage()).then((data) => {
		...
		if (
			batchDuplicates <= 15 ||
			(nextTMDBpage() - 1) * dbPageSize >= totalResultsTMDB
		) {
			if (pageNum == maxPageNumber()) {
				exploredTheEnd = true;
			}
...
function searchItemTemplate(index) {
	return (
		'<li class="movieSearchResultItem" onClick="chooseMovie(' +
		index +
		')">' +
		'<div class="row centerY spaceBetween movieSearchResultItemInner">' +
		'<img src="' +
		(movies[index].poster == null
			? "https://via.placeholder.com/95x140"
			: movies[index].poster) +
		'" class="movieSearchImg" />' +
		'<div class="column ml-3">' +
		'<div class="movieSearchItemHeader mb-1">Title:</div>' +
		'<div class="movieSearchItemText">' +
		movies[index].title +
		"</div>" +
		"</div>" +
		'<div class="column ml-3">' +
...
function refreshDbItem() {
	return (
		'<li class="movieSearchResultItem movieSearchItemEmpty" onClick="showMoreMovies()">' +
...
function generateDummyContent() {
	movieSearchResults.innerHTML =
		'<li class="movieSearchResultItem movieSearchItemEmpty movieSearchItemText row centerRow"><div>...</div></li>';
}
...
const getMovies = async () => {
	let response = await fetch(
		"//" + window.location.host + "/Cinema/getSomeMovies?",
		{
			method: "GET",
			mode: "cors"
		}
...
const countHowManyMoviesLike = async function (match) {
	let response = await fetch(
		"//" +
			...
			"/Cinema/countHowManyMoviesLike?match=" +
			match,
		{
			method: "GET",
			mode: "cors"
...
const getMoviesLikeInTMDB = async function (match, page) {
	let response = await fetch(
		"//" +
			window.location.host +
			"/Cinema/getMoviesLikeInTMDB?match=" +
			match +
			"&page=" +
			page,
		{
...
const addMovieIfNotExisting = async function (tmdbID) {
	fetch(
		"//" +
			window.location.host +
			"/Cinema/addMovieIfNotExisting?tmdbID=" +
			tmdbID,
		{
			method: "GET",
			mode: "cors"
